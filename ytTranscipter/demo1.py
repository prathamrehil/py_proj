    ...
    try:
        ...
        for entry in transcript_data:
            ...
            if current_line.endswith(('.', '!', '?')):
                current_paragraph += current_line + ' '  # Append to the current paragraph
                paragraphs.append(current_paragraph.strip())  # Append the completed paragraph
                keyframes.append(timestamp)
                current_paragraph = ""  # Reset for the next paragraph
            else:
                current_paragraph += current_line + ' '

        # Check if there's any remaining text after the last timestamp
